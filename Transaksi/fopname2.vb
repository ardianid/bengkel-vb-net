...
Public Class fopname2
    ...
    Private Function cekbukti(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As String
        ...
        sql = String.Format("select max(nobukti) from tr_opname where nobukti like '{0}%'", String.Format("{0}{1}{2}", nobuktiawal, tahun, bulan))
    ...
    Private Sub simpan()
        ...
        Try
            ...
            If addstat Then
                ...
                '1 . update faktur
                Dim sqlin_faktur As String = String.Format("insert into tr_opname (nobukti,tanggal,note) values('{0}','{1}','{2}')", _
                                                           tbukti.Text.Trim, convert_date_to_eng(ttgl.EditValue), tket.Text.Trim)
            ...
            Else
                ...
                '1. update faktur
                Dim sqlup_faktur As String = String.Format("update tr_opname set tanggal='{0}',note='{1}' where nobukti='{2}'", _
                                                          convert_date_to_eng(ttgl.EditValue), tket.Text.Trim, tbukti.Text.Trim)
    ...
    Private Function simpan2(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As Boolean
        ...
        For i As Integer = 0 To dv1.Count - 1
            ...
            If dv1(i)("noid") <> 0 Then
                ...
                Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, ttgl.EditValue, dv1(i)("kd_barang").ToString, 0, qty_sel, "Opname")


                ''3. insert to hist stok
                Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, ttgl.EditValue, dv1(i)("kd_barang").ToString, qty_sel, 0, "Opname")
            ...
            If dv1(i)("noid") = 0 Then
                ...
                '2. update barang
                Dim hasilplusmin As String = PlusMin_Barang_Adj(cn, sqltrans, qty_r, qty_sel, dv1(i)("kd_barang").ToString, True)
    ...
    Private Sub btdel_Click(sender As System.Object, e As System.EventArgs) Handles btdel.Click
        ...
        Else
            ...
            Try
                ...
                '2. update barang
                Dim hasilplusmin As String = PlusMin_Barang_Adj(cn, sqltrans, qtykecil_akh, qtykecil, kdbar, False)
                If Not hasilplusmin.Equals("ok") Then
                    MsgBox(hasilplusmin, vbOKOnly + vbExclamation, "Informasi")
                    sqltrans.Rollback()
                    GoTo langsung
                ...
                If addstat = False Then
                    ...
                    If DateValue(tgl_old) <> DateValue(ttgl.EditValue) Then
                        Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, tgl_old, kdbar, 0, qtykecil, "Opname (Edit)")
                        Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, ttgl.EditValue, kdbar, qtykecil, 0, "Opname (Edit)")
                    End If

                End If

                '3. insert to hist stok
                Clsmy.Insert_HistBarang(cn, sqltrans, tbukti.Text.Trim, ttgl.EditValue, kdbar, 0, qtykecil, "Opname (Edit)")
