...
Public Class fbeli
    ...
    Private Sub opendata()
        ...
        Dim sql As String = String.Format("SELECT tr_beli.nobukti, tr_beli.no_nota,tr_beli.nobukti_ps, tr_beli.tanggal, tr_beli.tanggal_beli,ms_penjual.kode as kd_toko, ms_penjual.nama AS nama_toko, ms_penjual.alamat AS alamat_toko, tr_beli.total_nett,tr_beli.sbatal,tr_beli.sbatal,tr_beli.nopol,tr_beli.beliperbaikan " & _
    ...
    Private Sub cari()
        ...
        sql = "SELECT tr_beli.nobukti, tr_beli.no_nota,tr_beli.nobukti_ps, tr_beli.tanggal, tr_beli.tanggal_beli,ms_penjual.kode as kd_toko, ms_penjual.nama AS nama_toko, ms_penjual.alamat AS alamat_toko, tr_beli.total_nett,tr_beli.sbatal,tr_beli.sbatal,tr_beli.nopol,tr_beli.beliperbaikan " & _
        ...
        Select Case tcbofind.SelectedIndex
            ...
            Case 2
                sql = String.Format("{0} where tr_beli.nobukti_ps like '%{1}%'", sql, tfind.Text.Trim)
            Case 3
    ...
    Private Sub hapus()
        ...
        Try
            ...
            If dv1(Me.BindingContext(bs1).Position)("nobukti_ps").ToString.Length > 0 Then
                ...
                Dim sqlcekpesan As String = String.Format("select nobukti from tr_beli where sbatal=0 and nobukti_ps='{0}'", dv1(Me.BindingContext(bs1).Position)("nobukti_ps").ToString)
                Dim cmdcekpesan As OleDbCommand = New OleDbCommand(sqlcekpesan, cn, sqltrans)
                Dim drdcekpesan As OleDbDataReader = cmdcekpesan.ExecuteReader

                Dim pesanada As Boolean = False
                If drdcekpesan.Read Then
                    If Not drdcekpesan(0).ToString.Equals("") Then
                        pesanada = True
                    End If
                End If
                drdcekpesan.Close()

                If pesanada = False Then
    ...
    Private Function hapus2(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As Boolean
        ...
        Dim pesanjasa As Boolean
        If Integer.Parse(dv1(bs1.Position)("beliperbaikan").ToString) = 0 Then
            pesanjasa = False
        Else
            pesanjasa = True
        End If
        ...
        If pesanjasa Then
            jdulhis_del = "Pembelian+Perbaikan (Btl)"
        Else
            jdulhis_del = "Pembelian (Btl)"
        End If
        ...
        If drd.HasRows Then
            While drd.Read
                ...
                If IsNumeric(drd("noid").ToString) Then
                    ...
                    If pesanjasa Then
                        Clsmy.Insert_HistBarang(cn, sqltrans, nobukti, tanggal, kdbarang, qtykecil, 0, jdulhis_del, nopol)
                        Clsmy.Insert_HistBarang(cn, sqltrans, nobukti, tanggal, kdbarang, 0, qtykecil, jdulhis_del, nopol)
                    Else
    ...
    Private Sub tsprint_Click(sender As System.Object, e As System.EventArgs) Handles tsprint.Click
        ...
        Dim nobukti As String = dv1(bs1.Position)("nobukti").ToString
        Dim beliperbaikan As Integer = Integer.Parse(dv1(bs1.Position)("beliperbaikan").ToString)
        ...
        Using fkar2 As New fpr_buktibeli With {.WindowState = FormWindowState.Maximized, .sql1 = sql1, .beliperbaikan = beliperbaikan}
            fkar2.ShowDialog(Me)
        End Using
    ...
    Private Sub tsprint2_Click(sender As System.Object, e As System.EventArgs) Handles tsprint2.Click
        ...
        Try
            ...
            If Integer.Parse(dv1(bs1.Position)("beliperbaikan").ToString) <> 1 Then
    ...
    Private Sub tcbofind_Click(sender As Object, e As EventArgs) Handles tcbofind.Click
